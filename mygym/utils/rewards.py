...
class ComfortConstrainedEnergyReward(LinearReward):
    """
    Lexicographic reward for minimizing HVAC energy under a comfort constraint.

    The energy objective is active only when all configured zone temperatures are
    within the comfort band, up to ``comfort_violation_tolerance``. If comfort is
    violated, the reward focuses on reducing the comfort constraint violation.
    """
    ...
    def __init__(
        ...
        lambda_energy: float = 1.0,
        lambda_temperature: float = 1.0,
        comfort_violation_tolerance: float = 0.0,
        comfort_violation_power: float = 2.0,
        include_energy_when_violated: bool = False
    ):
        super(ComfortConstrainedEnergyReward, self).__init__(
            temperature_variables,
            energy_variables,
            range_comfort_winter,
            range_comfort_summer,
        ...
        self.comfort_violation_tolerance = comfort_violation_tolerance
        self.comfort_violation_power = comfort_violation_power
        self.include_energy_when_violated = include_energy_when_violated

    def __call__(self, obs_dict: Dict[str, Any]
                 ) -> Tuple[float, Dict[str, Any]]:
        """Calculate a constrained objective reward.

        The reward implements:

        - feasible comfort state: minimize energy only;
        - infeasible comfort state: minimize comfort violation first.
        """
        try:
            assert all(temp_name in list(obs_dict.keys())
                       for temp_name in self.temp_names)
        ...
        reward, energy_term, comfort_term, constraint_violation = self._get_constrained_reward(
            energy_penalty, total_temp_violation)

        comfort_satisfied = constraint_violation <= 0.0
        reward_terms = {
            'energy_term': energy_term,
            'comfort_term': comfort_term,
            'reward_weight': self.W_energy if comfort_satisfied else 0.0,
            'abs_energy_penalty': energy_penalty,
            'abs_comfort_penalty': comfort_penalty,
            'total_power_demand': energy_consumed,
            'total_temperature_violation': total_temp_violation,
            'comfort_constraint_violation': constraint_violation,
            'comfort_violation_tolerance': self.comfort_violation_tolerance,
            'comfort_satisfied': comfort_satisfied,
            'constrained_objective': 'energy' if comfort_satisfied else 'comfort'
        }

        return reward, reward_terms

    def _get_constrained_reward(
        self,
        energy_penalty: float,
        total_temp_violation: float
    ) -> Tuple[float, float, float, float]:
        constraint_violation = max(
            0.0,
            total_temp_violation - self.comfort_violation_tolerance)
        comfort_satisfied = constraint_violation <= 0.0

        energy_term = self.lambda_energy * self.W_energy * energy_penalty
        if not comfort_satisfied and not self.include_energy_when_violated:
            energy_term = 0.0

        comfort_term = 0.0
        if not comfort_satisfied:
            comfort_term = -self.lambda_temp * \
                (constraint_violation ** self.comfort_violation_power)

        reward = energy_term + comfort_term
        return reward, energy_term, comfort_term, constraint_violation
